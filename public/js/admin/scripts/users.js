$(document).ready(function(){
	...
	$('button[name=save]').click(function(e){
		e.preventDefault();
		var validation = validate('form[name=user]');
		if(validation){
			...
			$.ajax({
				...
				error:	function(jqXHR){
					showError(jqXHR.responseText, type+'::/admin/users'+id);
				},
				success:function(data){
					try{
						...
						showError(e+data, type+'::/admin/users'+id);
					}
				}
			});
		}else{
		...
		$(document).on('customEvent', function(e){
			if(e.message === true){
				...
				$.ajax({
					...
					headers:{'X-CSRF-TOKEN': $('meta[name=csrf-token]').attr('content')},
					error:	function(jqXHR){
						showError(jqXHR.responseText, 'DELETE::/admin/users/'+id);
					},
					success:function(data){
						try{
							data = JSON.parse(data);
							...
							showError(e + data, 'DELETE::/admin/users/'+id);
						}
					}
				})
			}
